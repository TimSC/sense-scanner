...
unsigned int Workspace::AddSource(QString &fina, QString UidStr, class AvBinMedia *mediaIn)
{
    ...
    std::tr1::shared_ptr<class AnnotThread> annotThread(new class AnnotThread(&*ann, mediaIn));
...
unsigned int Workspace::AddAutoAnnot(QString annotUid, QString algUid, class AvBinMedia *mediaIn)
{
...
void Workspace::Load(QString fina, class AvBinMedia *mediaBackIn)
{
    this->Clear();
        ...
        if(!e.isNull()) {
            ...
            if(e.tagName() == "sources")
            {
                ...
                while(!sourceNode.isNull())
                {
                    ...
                    std::tr1::shared_ptr<class AnnotThread> annotThread(new class AnnotThread(&*ann, mediaBackIn));
                    annotThread->SetEventLoop(this->eventLoop);
                    annotThread->Start();
                    ann->annotThread = annotThread;
